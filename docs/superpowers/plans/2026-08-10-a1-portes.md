...
| `src/app/creer-mon-entreprise/{page,actions}.tsx` | *(créé)* SIRET → confirmation → adresse → lien |
| `src/app/creer-mon-compte/{page,actions}.tsx` | *(créé)* Adresse + nom → lien |
...
### Task 6 : `/creer-mon-entreprise` — le SIRET d'abord
...
import { lookupForSignUp, recordCompanyIntent, type LookupState } from './actions'

const initialState: LookupState = {}

/**
 * L'inscription de l'artisan, en deux temps.
 *
 * Le SIRET d'abord, l'adresse ensuite : voir le commentaire de
 * `lookupForSignUp`. La creation, elle, n'a lieu qu'a l'atterrissage du lien —
 * cette page n'ecrit qu'une intention.
 */
export default function SignUpCompanyPage() {
  const [state, lookup, pending] = useActionState(lookupForSignUp, initialState)
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      await recordCompanyIntent(email, state.found!.siret)
    ...
    return (
      <PublicShell variant="plain">
        ...
        >
          <Icon name="check" />
          <Text as="span">
            Lien envoyé à <strong>{email}</strong>. Votre entreprise sera créée à l’ouverture.
          </Text>
        </div>
      </PublicShell>
    )
    ...
    return (
      <PublicShell variant="plain">
        <div className="flex flex-col gap-2">
          <Heading level={1}>C’est bien vous ?</Heading>
          <Text size="sm" tone="soft">
            Ces informations viennent du répertoire officiel des entreprises.
          </Text>
        </div>

        <div className="flex flex-col gap-1 rounded-card border border-rule bg-card px-5 py-4">
          <Text as="p">
            <strong>{state.found.legalName}</strong>
          </Text>
          {state.found.legalFormLabel && (
            <Text size="sm" tone="soft">
              {state.found.legalFormLabel}
            </Text>
          )}
          <Text size="sm" tone="soft">
            {state.found.addressLine1}, {state.found.postalCode} {state.found.city}
          </Text>
          {state.found.rge && (
            <Text size="sm" tone="soft">
        ...
        <form onSubmit={submit} className="flex flex-col gap-5">
          <Field label="Votre e-mail" help="Pas de mot de passe : on vous envoie un lien." required>
            {(p) => (
              <Input
                {...p}
                type="email"
  ...
  return (
    <PublicShell variant="plain">
      ...
      <form action={lookup} className="flex flex-col gap-5">
        <Field label="SIRET" help="14 chiffres, espaces acceptés." error={state.error} required>
          {(p) => (
...
- [ ] **Step 3 : Vérifier que `Establishment` est bien exporté**

Run: `grep -n "export interface Establishment" src/services/company-lookup.ts`
Expected: une ligne. (Confirmé lors du cadrage.)
...
git commit -m "feat: l inscription artisan, le SIRET avant l adresse"
```
...
export default function SignUpRequesterPage() {
  ...
  return (
    <PublicShell variant="plain">
      <div className="flex flex-col gap-2">
        <Heading level={1}>Votre compte</Heading>
        <Text size="sm" tone="soft">
          Pour garder trace de vos travaux et des artisans que vous connaissez.
        </Text>
      </div>
...
test('les portes', async ({ page, browser }) => {
  ...
  await test.step('l’artisan voit son entreprise AVANT de donner son adresse', async () => {
    await page.goto('/creer-mon-entreprise')
    await page.getByLabel('SIRET').fill(SIRET)
    await page.getByRole('button', { name: 'Continuer' }).click()

    await expect(page.getByRole('heading', { name: 'C’est bien vous ?' })).toBeVisible()
  })

  await test.step('puis reçoit son lien, et son entreprise est créée à l’ouverture', async () => {
    await page.getByLabel('Votre e-mail').fill(ARTISAN)
    await page.getByRole('button', { name: 'Recevoir le lien' }).click()
  ...
  await test.step('le particulier s’inscrit seul et atterrit sur son répertoire', async () => {
    ...
    await client.getByRole('button', { name: 'Recevoir le lien' }).click()
  ...
  await test.step('une entreprise déjà inscrite est refusée AVANT toute adresse', async () => {
    ...
    await expect(second.getByLabel('Votre e-mail')).toHaveCount(0)
  })
